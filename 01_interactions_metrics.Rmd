...
pageviews_all_monthly <- pageviews_all %>%
  ...
  group_by(date) %>%
  summarise(monthly_views = sum(as.numeric(total))/1E9) %>%
  arrange(date) %>%
...
p <- ggplot(pageviews_all_monthly, aes(x=date, y = monthly_views)) +  
  ...
  geom_text(aes(x=as.Date('2018-04-01'), y=17, label="Page previews deployment completed"), size=3.5, vjust = -1.2, angle = 90, color = "black") +
  scale_y_continuous("pageviews per month (in billions)", labels = polloi::compress) +
  scale_x_date("Date", labels = date_format("%b %Y"), date_breaks = "2 months", limits= as.Date(c("2017-01-01", "2019-04-01"))) +
...
p <- ggplot(pageviews_all_monthly_fy, aes(x=MonthN, y = monthly_views, group = fiscal_year, color = fiscal_year, linetype = fiscal_year)) +  
  ...
  geom_vline(xintercept = 10,
             linetype = "dashed", color = "black") +
  geom_text(aes(x=10, y=15.5, label="Page previews deployment completed (April 2018)"), size=4, vjust = -1.2, angle = 90, color = "black") +
  scale_y_continuous("pageviews per month (in billions)", labels = polloi::compress) +
  scale_x_discrete(breaks = pageviews_all_monthly_fy$MonthN, labels = pageviews_all_monthly_fy$Month )+
  labs(title = "Wikimedia pageviews year-over-year comparison, July 2017-April 2019") +
...
previews_all_daily <- previews %>% 
  filter(date >= '2018-04-17' & date <= '2019-04-30')  #Filter out dates that occured before page previews rollout was completed.

p <- ggplot(previews_all_daily, aes(x=date, y = (previews_seen/1000000))) +  
  geom_line(color = 'red', size = 1) +
  geom_vline(xintercept = as.numeric(as.Date("2018-04-17")),
             linetype = "dashed", color = "black")+
  geom_vline(xintercept = as.numeric(as.Date("2018-12-25")),
             linetype = "dashed", color = "black")+
  geom_text(aes(x=as.Date('2018-12-25'), y=55, label="Christmas"), size=4, vjust = -1.2, angle = 90, color = "black") +
  geom_text(aes(x=as.Date('2018-04-17'), y=55, label="Page previews deployment completed"), size=4, vjust = -1.2, angle = 90, color = "black") +
  scale_y_continuous("seen previews (shown for at least 1 sec) in millions per day") +
...
p <- ggplot(interactions, aes(x=date, y = monthly_views, fill = forcats::fct_rev(type))) +  
  ...
  geom_vline(xintercept = as.numeric(as.Date("2018-04-01")),
             linetype = "dashed", color = "blue") +
  geom_text(aes(x=as.Date('2018-04-01'), y=8, label="Page Previews Deployment Completed"), size=4, vjust = -1.2, angle = 90, color = "black") +
